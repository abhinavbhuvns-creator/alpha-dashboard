...
# Force all dates to uniform UTC, then strip the timezone completely
...
# --- ASSIGN ALL SCANNERS (FutureWarning fixes applied) ---
scanner_conditions = {
    ...
    try:
        ...
        for tkr, details in summary_data.items():
            if isinstance(details, dict):
                # 🟢 THE FIX: Force keys to uppercase to match our stock list
                mcap_cache[tkr.upper()] = details.get('marketCap', 0)
            else:
                mcap_cache[tkr.upper()] = 0
    except Exception as e:
